...
import { APARTMENT_DPE_ONLY_PRICES, ERP_OPTION_PRICE, getPackPrice, HOUSE_DPE_ONLY_PRICES, PackPurpose } from '@/lib/property-pricing'
...
export default function DevisPage(){
 ...
 const erpFree=!(missionMinimale&&hasSurfaceAttestation)
 const erpOptionPrice=missionMinimale&&hasSurfaceAttestation&&erpOptionChecked?ERP_OPTION_PRICE:0
 const base=quoteOnRequest?0:(missionMinimale?missionBase:getPackPrice(form.propertyType,effectivePack,sizeIndex,packPurpose)??0),measurement=!missionMinimale&&form.propertyType==='house'&&form.measurement!=='none'&&!quoteOnRequest?HOUSE_SIZE_TIERS[sizeIndex]?.measurementPrice||0:0,assainissement=form.assainissement?(form.propertyType==='apartment'?APARTMENT_ASSAINISSEMENT_PRICE:HOUSE_ASSAINISSEMENT_PRICE):0,proximity=form.proximity?-10:0
 ...
 const lines=useMemo(()=>{if(quoteOnRequest)return [{label:QUOTE_ON_REQUEST_MESSAGE,ttc:0}];const out=[missionMinimale?{label:hasSurfaceAttestation?'Mission minimale : DPE seul':'Mission minimale : DPE + attestation de surface (pack 2)',ttc:base}:{label:`Pack ${effectivePack} diagnostics – ${form.propertyType==='apartment'?apartmentLabels[sizeIndex]:houseLabels[sizeIndex]}`,ttc:base}];if(erpFree)out.push({label:'ERP (état des risques et pollutions) — 25 € offert',ttc:0});if(erpOptionPrice>0)out.push({label:'ERP (état des risques et pollutions) — option',ttc:erpOptionPrice});if(!missionMinimale&&form.propertyType==='house'&&form.measurement==='boutin')out.push({label:'Mesurage (surface habitable)',ttc:measurement});if(!missionMinimale&&form.propertyType==='house'&&form.measurement==='attestation')out.push({label:'Attestation de mesurage',ttc:measurement});if(form.assainissement)out.push({label:'Contrôle de l’assainissement',ttc:assainissement});if(form.proximity)out.push({label:'Remise proximité Alfortville / Maisons-Alfort',ttc:-10});if(promo)out.push({label:`Code pro ${promo.code}${promo.label?' – '+promo.label:''}`,ttc:-promoDiscount});return out},[base,measurement,erpFree,erpOptionPrice,assainissement,missionMinimale,hasSurfaceAttestation,form.measurement,form.assainissement,form.proximity,form.propertyType,sizeIndex,effectivePack,promo,promoDiscount,quoteOnRequest])
 ...
 const quoteKind=quoteOnRequest?`pack_${effectivePack}_sur_devis`:missionMinimale?(hasSurfaceAttestation?'mission_minimale_dpe_seul':'mission_minimale_pack2'):`pack_${effectivePack}`
 ...
 return <AppShell active="devis"><main className="page" style={{maxWidth:1280}}>
  ...
  <div style={{display:'grid',gridTemplateColumns:'minmax(0,1.35fr) minmax(320px,.65fr)',gap:18,alignItems:'start'}}>
   <section className="card" style={{padding:22}}><h2 style={{marginTop:0,color:'#062b59'}}>Nouveau devis</h2>{error&&<div className="error">{error}</div>}<div className="edit-grid">
    ...
    <label className="edit-field"><span>Type de bien</span><select value={form.propertyType} onChange={e=>{setForm(f=>({...f,propertyType:e.target.value as PropertyType,packCount:2,sizeKey:'0',measurement:'none'}));setHasSurfaceAttestation(false);setErpOptionChecked(false)}}><option value="apartment">Appartement</option><option value="house">Maison</option></select></label>
    ...
    <label className="edit-field"><span>Nombre de diagnostics</span><select value={effectivePack} disabled={missionMinimale} onChange={e=>setForm(f=>({...f,packCount:Number(e.target.value)}))}>{allowedPacks.map(n=><option key={n} value={n}>{n} diagnostics</option>)}</select></label><div/>
    <label className="check-field"><input type="checkbox" checked={missionMinimale} onChange={e=>{setMissionMinimale(e.target.checked);if(!e.target.checked)setErpOptionChecked(false)}}/><span>Mission minimale (DPE seul)</span></label>
