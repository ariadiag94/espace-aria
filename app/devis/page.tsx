...
export default function DevisPage(){
 ...
 const load=async()=>{const {data:{session}}=await supabase.auth.getSession();if(!session){router.replace('/login');return}const [d,q]=await Promise.all([supabase.from('dossiers').select('id,dossier_name').order('created_at',{ascending:false}),supabase.from('quotes').select('id,quote_number,status,total_ht,total_vat,total_ttc,created_at,dossier_id').order('created_at',{ascending:false}).limit(20)]);if(d.data){setDossiers(d.data as Dossier[]);setForm(f=>({...f,dossier_id:f.dossier_id||d.data?.[0]?.id||''}))}if(q.data)setQuotes(q.data as Quote[]);setLoading(false)}
 useEffect(()=>{void load()},[])
 const sizeIndex=Math.max(0,Math.min(4,Number(form.sizeKey)||0)),packTable=form.propertyType==='apartment'?apartmentPacks:housePacks,allowedPacks=form.propertyType==='apartment'?[2,3,4,5,6,7]:[2,3,4,5,6],effectivePack=allowedPacks.includes(form.packCount)?form.packCount:allowedPacks[0]
 const base=packTable[effectivePack]?.[sizeIndex]||0,boutin=form.boutin?(form.propertyType==='apartment'?70:boutinHouse[sizeIndex]):0,assainissement=form.assainissement?(form.propertyType==='apartment'?100:160):0,proximity=form.proximity?-10:0,totalTtc=Math.max(0,base+boutin+assainissement+proximity),totalHt=Math.round(totalTtc/1.2*100)/100,vat=Math.round((totalTtc-totalHt)*100)/100
 const lines=useMemo(()=>{const out=[{label:`Pack ${effectivePack} diagnostics – ${form.propertyType==='apartment'?apartmentLabels[sizeIndex]:houseLabels[sizeIndex]}`,ttc:base}];if(form.boutin)out.push({label:'Option Loi Boutin',ttc:boutin});if(form.assainissement)out.push({label:'Option assainissement',ttc:assainissement});if(form.proximity)out.push({label:'Remise proximité Alfortville / Maisons-Alfort',ttc:-10});return out},[base,boutin,assainissement,form.boutin,form.assainissement,form.proximity,form.propertyType,sizeIndex,effectivePack])
 ...
 const save=async()=>{setError('');if(!form.dossier_id){setError('Choisis un dossier.');return}setSaving(true);const quoteNumber=await createQuoteNumber();const {data:q,error:qError}=await supabase.from('quotes').insert({dossier_id:form.dossier_id,quote_number:quoteNumber,status:'draft',total_ht:totalHt,total_vat:vat,total_ttc:totalTtc,property_type:form.propertyType,property_size:form.propertyType==='apartment'?apartmentLabels[sizeIndex]:houseLabels[sizeIndex],quote_kind:`pack_${effectivePack}`,notes:form.notes||null}).select('id').single();if(qError||!q){setError(qError?.message||'Impossible de créer le devis.');setSaving(false);return}const {error:lError}=await supabase.from('quote_lines').insert(lines.map((l,i)=>({quote_id:q.id,label:l.label,quantity:1,unit_ttc:l.ttc,total_ttc:l.ttc,sort_order:i})));if(lError){setError(lError.message);setSaving(false);return}setSaving(false);router.push(`/devis/${q.id}`)}
 ...
 return <AppShell active="devis"><main className="page" style={{maxWidth:1280}}>
  ...
  <div style={{display:'grid',gridTemplateColumns:'minmax(0,1.35fr) minmax(320px,.65fr)',gap:18,alignItems:'start'}}>
   <section className="card" style={{padding:22}}><h2 style={{marginTop:0,color:'#062b59'}}>Nouveau devis</h2>{error&&<div className="error">{error}</div>}<div className="edit-grid">
    ...
    <label className="edit-field"><span>Nombre de diagnostics</span><select value={effectivePack} onChange={e=>setForm(f=>({...f,packCount:Number(e.target.value)}))}>{allowedPacks.map(n=><option key={n} value={n}>{n} diagnostics</option>)}</select></label><div/>
    <label className="check-field"><input type="checkbox" checked={form.boutin} onChange={e=>setForm(f=>({...f,boutin:e.target.checked}))}/><span>Loi Boutin en option</span></label><label className="check-field"><input type="checkbox" checked={form.assainissement} onChange={e=>setForm(f=>({...f,assainissement:e.target.checked}))}/><span>Assainissement</span></label><label className="check-field"><input type="checkbox" checked={form.proximity} onChange={e=>setForm(f=>({...f,proximity:e.target.checked}))}/><span>Remise proximité -10 €</span></label>
   ...
   <aside className="card" style={{padding:22,position:'sticky',top:18}}><div className="eyebrow">APERÇU</div><h2 style={{margin:'4px 0 14px',color:'#062b59'}}>Montant du devis</h2><div style={{display:'grid',gap:9}}>{lines.map((l,i)=><div key={i} style={{display:'flex',justifyContent:'space-between',gap:12,borderBottom:'1px solid #edf2f7',paddingBottom:8}}><span>{l.label}</span><b>{euro(l.ttc)}</b></div>)}</div><div style={{marginTop:16,paddingTop:14,borderTop:'2px solid #dce6f0'}}><div style={{display:'flex',justifyContent:'space-between'}}><span>HT</span><b>{euro(totalHt)}</b></div><div style={{display:'flex',justifyContent:'space-between',marginTop:6}}><span>TVA 20 %</span><b>{euro(vat)}</b></div><div style={{display:'flex',justifyContent:'space-between',marginTop:10,fontSize:24,color:'#062b59'}}><strong>TTC</strong><strong>{euro(totalTtc)}</strong></div></div><button className="action-btn primary-action" style={{width:'100%',marginTop:18}} disabled={saving} onClick={save}>{saving?'Création…':'Créer le brouillon'}</button><p style={{fontSize:12,color:'#6f7d90',marginBottom:0}}>Le devis reste en brouillon : aucune transmission automatique.</p></aside>
  </div>
