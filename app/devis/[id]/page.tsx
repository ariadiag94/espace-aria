...
type Dossier={id:string;dossier_name:string;status?:string|null}
...
export default function QuoteDetailPage(){
 ...
 const load=async()=>{
  ...
  const [d,l]=await Promise.all([supabase.from('dossiers').select('id,dossier_name,status').eq('id',q.dossier_id).single(),supabase.from('quote_lines').select('*').eq('quote_id',id).order('sort_order',{ascending:true})])
 ...
 const save=async()=>{
  if(!quote)return;setSaving(true);setError('')
 ...
 return <AppShell active="devis"><main className="page" style={{maxWidth:1240}}>
  ...
  <div style={{display:'grid',gridTemplateColumns:'minmax(0,1.45fr) minmax(300px,.55fr)',gap:18,alignItems:'start'}}>
   <section className="card" style={{padding:22}}>
    ...
    <div style={{display:'flex',justifyContent:'flex-end',marginTop:16}}><button className="action-btn primary-action" disabled={saving} onClick={save}>{saving?'Enregistrement…':'Enregistrer les modifications'}</button></div>
   </section>
   <aside style={{display:'grid',gap:14,position:'sticky',top:18}}>
    <section className="card" style={{padding:20}}><div className="eyebrow">DOSSIER</div><h2 style={{margin:'4px 0 10px',color:'#062b59'}}>{dossier?.dossier_name||'Dossier'}</h2><p style={{margin:'0 0 10px',color:'#6f7d90'}}>{quote.property_type==='house'?'Maison':quote.property_type==='apartment'?'Appartement':'Bien'} {quote.property_size?`· ${quote.property_size}`:''}</p><Link className="back" href={`/dossiers/${quote.dossier_id}`}>Ouvrir le dossier →</Link></section>
