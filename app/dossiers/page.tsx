...
export default function DossiersPage(){
  ...
  return <AppShell active="dossiers"><main className="page"><div className="hero-row"><div><h1>Dossiers</h1><p>Recherche et suivi des interventions ARIA.</p></div></div>{loading?<div className="loading">Chargement…</div>:<div className="card table-card"><div className="table-head"><div>Dossier</div><div>Statut</div><div>Liciel</div><div></div></div>{rows.map(r=><Link href={`/dossiers/${r.id}`} key={r.id} className="table-row"><div><b>{r.dossier_name}</b><div className="subline">{r.purpose||'—'}</div></div><div><span className="status">{r.status.replaceAll('_',' ')}</span></div><div>{r.liciel_number||'—'}</div><div className="back">Ouvrir</div></Link>)}</div>}</main></AppShell>
}
